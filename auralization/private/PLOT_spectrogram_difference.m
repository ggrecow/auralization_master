function PLOT_spectrogram_difference(input_mono, input_stereo, fs, tag_title, tag_auralization, tag_save)
...
for nChannels = 1:size( input_stereo, 2 )

    ax = nexttile(nChannels);

    [P,F,T] = myspecgram(input_stereo(:,nChannels), fs, windowsize , 0.75); % overlap is 75% of nfft here

    SPL = 20.*log10(abs(P)./pref) - 20.*log10(abs(P_mono)./pref);
    ...
    colormap( ax, flipud(cMap.cmap_divergent) );
...
zString = '$L_{\mathrm{Z,stereo}} - L_{\mathrm{Z,mono}}$ (dB re 20$~\mu$Pa)';
