...
idx_upper = round( idx_min_dist + TrimTimebins); % idx of TrimTime after the min. distance
...
if idx_lower < 1 || idx_upper > length(time) % if TrimTime is larger than the input signal's time
    ...
    fprintf( '\n*--------------------------------------------------------------------------*' );
    fprintf( '\nLog of trim function (time and distance based) \n' );
    fprintf( '\n- Min. distance between source and receiver: %.4g m', min_dist ) ;
    fprintf( '\n- SPL on min. distance point: %.4g dB SPL', SPL_vs_time(idx_min_dist) ) ;
    fprintf( '\n- Max. SPL on the data: %.4g dB SPL', max(SPL_vs_time) ) ;
